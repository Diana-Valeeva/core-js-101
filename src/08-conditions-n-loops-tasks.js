...
function doRectanglesOverlap(rect1, rect2) {
  if (rect1.top + rect1.height > rect2.top && rect1.width + rect1.left > rect2.left) {
    return true;
  }
  return false;
}
...
function isInsideCircle(circle, point) {
  const diff = Math.sqrt((point.x - circle.center.x) ** 2 + (point.y - circle.center.y) ** 2);
...
function getDigitalRoot(num) {
  ...
  const res = numArr.reduce((acc, elem) => acc + (+elem), 0);
  return num > 9 ? getDigitalRoot(res) : res;
}
